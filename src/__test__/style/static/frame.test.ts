...
describe('Static generating. Gradient pattern', () => {
  ...
  test('1.49 meter', () => {
    ...
    stand.changeStyle(style);
    stand.resize({ width: 1.49, height: 1, depth: 1 });

    expect(stand.dimension.width).toBe(1.49);
    const mainBox = stand.style.boxes[0];

    expect(mainBox.children.length).toBe(3);

    expect(mainBox.children[0].dimension.width).toBe(0.44);
    expect(mainBox.children[1].dimension.width).toBe(0.44);
  ...
  test('1.54 meter', () => {
    ...
    stand.changeStyle(style);
    stand.resize({ width: 1.54, height: 1, depth: 1 });

    expect(stand.dimension.width).toBe(1.54);
    const mainBox = stand.style.boxes[0];
    ...
    expect(mainBox.children[2].dimension.width).toBe(0.31);
    expect(mainBox.children[3].dimension.width).toBe(0.31);
  });
  ...
  test('1.60 meter', () => {
    ...
    stand.changeStyle(style);
    stand.resize({ width: 1.60, height: 1, depth: 1 });

    expect(stand.dimension.width).toBe(1.60);
    const mainBox = stand.style.boxes[0];
  ...
  test('1.70 meter', () => {
    ...
    stand.changeStyle(style);
    stand.resize({ width: 1.70, height: 1, depth: 1 });

    expect(stand.dimension.width).toBe(1.70);
    const mainBox = stand.style.boxes[0];
  ...
  test('1.77 meter', () => {
    ...
    stand.changeStyle(style);
    stand.resize({ width: 1.77, height: 1, depth: 1 });

    expect(stand.dimension.width).toBe(1.77);
    const mainBox = stand.style.boxes[0];
